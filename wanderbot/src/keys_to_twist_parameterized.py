...
g_last_twist = None
g_vel_scales = [0.1,0,1] #default is very slow
...
if __name__ == '__main__':
    ...
    if rospy.has_param('~linear_scale'):
        ...
        rospy.logwarn('linear scale not provided; default is %.1f'% g_vel_scales[1])
    if rospy.has_param('~angular_scale'):
        g_vel_scales[0] = rospy.get_param('~angular_scale')
    else:
        rospy.logwarn('angular scale not provided; default is %.1f'% g_vel_scales[0])
